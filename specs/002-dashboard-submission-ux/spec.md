...
- Submission succeeds but the user refreshes the confirmation page (the page should remain understandable and offer navigation).
...
- **FR-001**: When a user submits a transcription job from the dashboard via upload and the submission is accepted, the browser MUST display a human-friendly confirmation page (not raw JSON).
- **FR-002**: When a user submits a transcription job from the dashboard via direct video URL and the submission is accepted, the browser MUST display the same confirmation page pattern (not raw JSON or alert-only feedback).
...
- This feature adds only these user-facing pages: submission confirmation, friendly submission error, and job not found.
...
- Upload progress detail depends on whether the user’s browser can report upload progress; when exact progress is unavailable, an indeterminate indicator is acceptable.
